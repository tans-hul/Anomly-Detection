...
def get_prediction(json_data):
    ...
    top_features, contributions = zip(*sorted_contributions)
    is_anomaly = distance > 4.148080343148985

    # Normalizing contribution ratings to a scale of 1-10
    normalized_contributions = np.interp(contributions, (min(contributions), max(contributions)), (1, 10))

    top_feature_names = [data.columns[idx] for idx in top_features]
    kmeans_result = {
        "is_anomaly": bool(is_anomaly),
        "top_features": top_feature_names[:5],
        "contributions": normalized_contributions.tolist()[:5]
    }
...
def predict():
    ...
    result = [get_prediction(json_data) if isinstance(json_data, dict) else [get_prediction(data) for data in json_data]]
